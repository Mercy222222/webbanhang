...
const uiHtml = `
  ...
  <div id="desktop-ui" class="pointer-events-none fixed inset-0 z-40 font-sans">
    
    <!-- Music Player Widget -->
    <div id="widget-music" class="desktop-widget glass-panel flex flex-col pointer-events-auto" style="bottom: 30px; left: 30px; width: 340px; transform: scale(0.9); transform-origin: bottom left;">
      <!-- Drag Handle -->
    ...
    <!-- Equalizer Widget -->
    <div id="widget-eq" class="desktop-widget glass-panel flex flex-col pointer-events-auto" style="bottom: 30px; left: 360px; width: 340px; transform: scale(0.9); transform-origin: bottom left;">
      <div class="drag-handle w-full h-6 flex items-center justify-center border-b border-white/5 opacity-50 hover:opacity-100">
        <div class="w-10 h-1 rounded-full bg-white/20"></div>
      </div>
...
// Xóa UI cũ
let uiHtmlRegex = /<style>[\s\S]*?\.glass-panel[\s\S]*?id="desktop-ui"[\s\S]*?<\/script>/;
html = html.replace(uiHtmlRegex, '');

// Trong trường hợp Regex không bắt hết do DOM lồng nhau
html = html.replace(/<!-- GLASSMORPHISM DESKTOP UI OVERLAY -->[\s\S]*?<\/script>/, '');

// Inject mới
html = html.replace('</body>', uiHtml + '\n</body>');

fs.writeFileSync(path, html);
console.log('Successfully replaced UI with draggable, smaller widgets placed in optimal positions.');
