...
//#endregion
//#region node_modules/valibot/dist/index.cjs
...
//#endregion
//#region agents/simple_agent.ts
var import_dist = require_dist$1();
var simple_agent_exports = /* @__PURE__ */ __exportAll({ default: () => simple_agent_default });
var import_dist$1 = /* @__PURE__ */ __toESM(require_dist());
var readDocxTool = defineTool({
	name: "read_docx_file",
	description: "Đọc và trích xuất nội dung văn bản từ một file .docx.",
	parameters: import_dist$1.object({ filePath: import_dist$1.string("Đường dẫn tới file .docx cần đọc") }),
	execute: async ({ filePath }) => {
		try {
			return (await mammoth.extractRawText({ path: filePath })).value || "File rỗng hoặc không thể trích xuất văn bản.";
		} catch (error) {
			return `Lỗi khi đọc file docx: ${error.message}`;
		}
	}
});
var createPptxTool = defineTool({
	name: "create_pptx_presentation",
	description: "Tạo file slide thuyết trình PowerPoint (.pptx) dựa trên nội dung dàn ý.",
	parameters: import_dist$1.object({
		fileName: import_dist$1.string("Tên file .pptx muốn lưu (ví dụ: BaiThuyetTrinh.pptx)"),
		slides: import_dist$1.array(import_dist$1.object({
			title: import_dist$1.string("Tiêu đề của slide"),
			content: import_dist$1.array(import_dist$1.string(), "Các ý chính (bullet points) trong slide")
		}))
	}),
...
var { manifest, createdAgents, dispatchAgentNames, workflowHandlers, localWorkflowHandlers, agentRouteMiddleware, workflowRouteMiddleware, channelHandlers } = normalizeBuiltModules({ "simple_agent": simple_agent_exports }, {}, {});
...
var flueApp = createDefaultFlueApp();
...
if (isLocalCliMode && hasIpcChannel) {
	...
	console.log("[flue] Agents: simple_agent");
