...
export default function AdminDashboard() {
  ...
  // Fetch initial data
  const fetchData = async (urlBase: string) => {
    setLoading(true);
    setError(null);
    try {
      // 1. Fetch Stats
      const statsRes = await fetch(`${urlBase}api/stats`, { mode: "cors" });
      ...
      // 2. Fetch Orders
      const ordersRes = await fetch(`${urlBase}api/orders`, { mode: "cors" });
  ...
  const fetchOrderDetail = async (orderId: number) => {
    ...
    try {
      const res = await fetch(`${apiBase}api/orderDetail/${orderId}`, { mode: "cors" });
